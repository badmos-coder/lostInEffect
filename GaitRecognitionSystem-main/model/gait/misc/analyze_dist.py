...
def read(csv_path, dataset, metric):
    ...
    df = df1.drop(index=df1.index[drop_list])  
...
def data_fit(data, path):
    ...
    f = Fitter(data, distributions=distributions, timeout=10000)  
    f.fit()  
    ...
    print(f"f.df_errors:")
    print(f"{f.df_errors}")
    print(f"{f.fitted_param['logistic']=}")  
    print(f"{f.fitted_param['t']=}")  
    print(f"{f.fitted_param['johnsonsb']=}")  
    ...
    f.plot_pdf(Nbest=2, lw=2, method='sumsquare_error')  
